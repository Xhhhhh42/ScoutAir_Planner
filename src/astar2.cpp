...
void Astar::init( ros::NodeHandle& nh, std::shared_ptr<FrontierMap> &frontiermap ) 
{
  nh.param("astar/resolution_astar", resolution_, -1.0F );
  nh.param("astar/lambda_heu", lambda_heu_, -1.0F);
  nh.param("astar/max_search_time", max_search_time_, -1.0F );
  nh.param("astar/allocate_num", allocate_num_, -1);

  tie_breaker_ = 1.0 + 1.0 / 1000;

//   this->edt_env_ = env;
  frontiermap_ = frontiermap;

  /* ---------- map params ---------- */
  this->inv_resolution_ = 1.0 / resolution_;

  origin_ = frontiermap_->voxbloxmap_origin_;
...
void Astar::setResolution( const float& res ) 
{
  resolution_ = res;
  this->inv_resolution_ = 1.0 / resolution_;
}
...
int Astar::search(const Eigen::Vector3f& start_pt, const Eigen::Vector3f& end_pt) 
{
  ...
  while (!open_set_.empty()) {
    ...
    Eigen::Vector3f cur_pos = cur_node->position;
    Eigen::Vector3f nbr_pos;
    Eigen::Vector3f step;

    for (float dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (float dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (float dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          step << dx, dy, dz;
          if (step.norm() < 1e-3) continue;
          nbr_pos = cur_pos + step;
          // Check safety
          if( !frontiermap_->isInBox(nbr_pos) ) continue;
          Eigen::Vector3d nbr_pos_d = nbr_pos.cast<double>();
          if( frontiermap_->voxblox_map_->checkVoxelDistance(nbr_pos_d) != VoxbloxMap::VoxelStatus::kFree )
            continue;
        ...
        //     continue;
          ...
          float len = dir.norm();
          dir.normalize();
          for (float l = 0.1; l < len; l += 0.1) {
            Vector3f ckpt = cur_pos + l * dir;
            Eigen::Vector3d ckpt_d = ckpt.cast<double>();
            if( frontiermap_->voxblox_map_->checkVoxelDistance(ckpt_d) != VoxbloxMap::VoxelStatus::kFree )
            {
              safe = false;
              break;
            }
          ...
          NodePtr neighbor;
          float tmp_g_score = step.norm() + cur_node->g_score;
...
float Astar::getEarlyTerminateCost() {
...
float Astar::pathLength(const vector<Eigen::Vector3f>& path) {
  float length = 0.0;
...
void Astar::backtrack(const NodePtr& end_node, const Eigen::Vector3f& end) {
...
std::vector<Eigen::Vector3f> Astar::getPath() {
  return path_nodes_;
}

float Astar::getDiagHeu(const Eigen::Vector3f& x1, const Eigen::Vector3f& x2) {
  float dx = fabs(x1(0) - x2(0));
  float dy = fabs(x1(1) - x2(1));
  float dz = fabs(x1(2) - x2(2));
  float h;
...
float Astar::getManhHeu(const Eigen::Vector3f& x1, const Eigen::Vector3f& x2) {
  float dx = fabs(x1(0) - x2(0));
  float dy = fabs(x1(1) - x2(1));
  float dz = fabs(x1(2) - x2(2));
...
float Astar::getEuclHeu(const Eigen::Vector3f& x1, const Eigen::Vector3f& x2) {
  return tie_breaker_ * (x2 - x1).norm();
}

std::vector<Eigen::Vector3f> Astar::getVisited() {
  vector<Eigen::Vector3f> visited;
  for (int i = 0; i < use_node_num_; ++i)
    visited.push_back(path_node_pool_[i]->position);
  return visited;
}

void Astar::posToIndex(const Eigen::Vector3f& pt, Eigen::Vector3i& idx) {
