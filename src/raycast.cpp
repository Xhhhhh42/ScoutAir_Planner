...
void Raycast(const Eigen::Vector3f& start, const Eigen::Vector3f& end, const Eigen::Vector3f& min,
  ...
  while (true) {
    if (x >= min.x() && x < max.x() && y >= min.y() && y < max.y() && z >= min.z() && z < max.z()) {
      ...
      dist = (Eigen::Vector3f(x, y, z) - start).squaredNorm();
...
bool RayCaster::setInput(const Eigen::Vector3f& start,
                         const Eigen::Vector3f& end /* , const Eigen::Vector3f& min,
...
bool RayCaster::step(Eigen::Vector3f& ray_pt) {
  // if (x_ >= min_.x() && x_ < max_.x() && y_ >= min_.y() && y_ < max_.y() &&
  // z_ >= min_.z() && z_ <
  // max_.z())
  ray_pt = Eigen::Vector3f(x_, y_, z_);

  // step_num_++;

  // dist_ = (Eigen::Vector3f(x_, y_, z_) - start_).squaredNorm();
...
void RayCaster::setParams(const float& res, const Eigen::Vector3f& origin) {
  resolution_ = res;
  half_ = Eigen::Vector3f(0.5, 0.5, 0.5);
  offset_ = half_ - origin / resolution_;
}

bool RayCaster::input(const Eigen::Vector3f& start, const Eigen::Vector3f& end) {
...
bool RayCaster::nextId(Eigen::Vector3i& idx) {
  auto tmp = Eigen::Vector3f(x_, y_, z_);
...
bool RayCaster::nextPos(Eigen::Vector3f& pos) {
  auto tmp = Eigen::Vector3f(x_, y_, z_);
